...
export const createPlaylist = async (req, res) => {
  ...
  try {
    ...
    const genre = req.body.genre; // check the genre
    const playlist_id = genrePlaylistMap[genre]; // set the genre for the request

    // get the source playlist data
    ...
    const runLengthMs = runLength * 60 * 1000; // spotify only deals in ms
    let totalDuration = 0;
    const selectedTrackUris = [];

    for (const track of sourceTracks) { //TODO: iterate through in a random order
    ...
    const newPlaylistRes = await fetch(`https://api.spotify.com/v1/users/${userId}/playlists`, {
      ...
      body: JSON.stringify({
        ...
        public: true, //important keep this true
      }),
    });
